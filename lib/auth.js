...
// Kosovo Labour Law / company policy: leave entitlement starts after the
...
// Returns the date from which the employee may start requesting leave, or null
// when no employment start date is on record.
export function leaveEligibleFrom(startDate) {
...
export function isLeaveEligible(startDate) {
  const from = leaveEligibleFrom(startDate);
  if (!from) return true;
