...
// Month by month picture of a single leave type for the yearly report.
export function typeYear(requests, type, year, userAllowance) {
  const today = startOfToday();
  const months = [];
  for (let i = 0; i < 12; i++) months.push(0);
  let used = 0;
  ...
  const allowance = allowanceFor(type, userAllowance);
  return {
    type: type,
    label: typeLabel(type),
    months: months,
    ...
    pending: pending,
    allowance: allowance,
    remaining: allowance === null ? null : allowance - used - planned
  };
}

// Month by month picture for every leave type at once - used by the yearly report.
export function allTypeYears(requests, year, userAllowance) {
  return LEAVE_TYPES.map(function (item) {
    return typeYear(requests, item.value, year, userAllowance);
  });
}

// Month by month vacation (PTO) picture for the yearly report.
export function vacationYear(requests, year, userAllowance) {
  return typeYear(requests, 'VACATION', year, userAllowance);
}
